...
namespace InterviewSelectionProcess.Controllers
{
    public class CandidateStatusController : Controller
    {
        ...
        //    if (!String.IsNullOrEmpty(SearchPost) && !String.IsNullOrEmpty(SearchTech) && !(SearchExperience==null))
          //  {
                candidateStatus = candidateStatus.Where(s =>( s.Candidate.Post.PostName.Contains(SearchPost) || String.IsNullOrEmpty(SearchPost)) && (s.Candidate.Technology.Technology1.Contains(SearchTech) || String.IsNullOrEmpty(SearchTech)) && (s.Candidate.Experience==SearchExperience|| SearchExperience == null)).OrderBy(x=>x.CandidateID);
            //}
